...
void PoseVisualizationWidget::rebuildGeometry()
{
    ...
    for (size_t index = 0; index < poseCount; ++index)
    {
        ...
        const cv::Matx33d boardToCamera =
                rotationMatrix(pose.rotationVector);
        // 固定标定板视图需要对外参求逆，得到相机在标定板坐标系中的位姿
...
void PoseVisualizationWidget::paintEvent(QPaintEvent *event)
{
    ...
    // 使用全部线段端点计算可见范围，让不同拍摄尺度的场景都能自动居中
    ...
    connect(ui_->poseTable, &QTableWidget::currentCellChanged, this,
        [this](int currentRow, int, int, int)
        {
            ...
            ui_->poseVisualization->setHighlightedPose(
                imageItem == nullptr ? -1 : imageItem->data(kPoseIndexRole).toInt());
        });
...
{
    // 表格按误差降序排列，但保存原始位姿下标以保持三维高亮对应正确
...
void PoseResultDialog::updateExcludeButton()
{
    ...
    excludeButton_->setText(
        checkedCount == 0 ? tr("排除所选并重新标定") : tr("排除所选并重新标定 (%1)").arg(checkedCount));
    excludeButton_->setToolTip(
        remainingCount < 3 ? tr("至少保留 3 张有效图片") : tr("从图片列表移除勾选项，并立即重新标定"));
}
