...
namespace
{
        ...
        std::vector<cv::Point2f> &corners)
    {
        ...
        {
            // 传统检测器的输出还需亚像素优化，SB 检测器本身已返回高精度角点
    ...
    {
        return boardType == CalibrationBoardType::Charuco ? "charuco" : "chess";
    }

    cv::String cameraModelName(CameraModel cameraModel)
    {
        return cameraModel == CameraModel::Fisheye ? "fisheye" : "pinhole";
    }

    QString cameraModelDisplayName(CameraModel cameraModel)
    {
        return cameraModel == CameraModel::Fisheye ? QStringLiteral("Fisheye") : QStringLiteral("Pinhole");
    }
    ...
    bool parseCameraMatrix(const cv::FileNode &node, cv::Mat &matrix)
    {
        ...
        if (!node.isSeq())
        {
            // OpenCV matrix 节点可直接反序列化；普通 YAML 序列由下方分支处理
            node >> matrix;
    ...
    {
        return method == CalibrationMethod::Classic ? "findChessboardCorners" : "findChessboardCornersSB";
    }
    ...
    {
        // 用户输入的是方格数，OpenCV 检测模式使用的是少一行、少一列的内角点数
        ...
        const CalibrationOptions &opts)
    {
        ...
        // 丢弃不能映射到当前标定板角点表的异常 ID，避免索引越界进入求解器
    ...
    QImage matToQImage(const cv::Mat &in)
    {
        ...
        if (in.channels() == 1)
        {
            // QImage 必须持有自己的像素副本，不能依赖局部 cv::Mat 的生命周期
    ...
    bool showUndistorted, bool *found)
{
    ...
    // 预览从 GUI 事件链调用，OpenCV 异常在此转换为空图，不能越过 Qt 事件边界
    try
    {
        ...
        if (!cacheHit)
        {
            // 文件元数据和检测选项共同构成缓存键，覆盖磁盘替换与参数调整两类失效
            cv::Mat image =
            ...
            const BoardDetection detection =
                    validateOptions(opts).isEmpty() ? detectBoard(image, opts) : BoardDetection{};
    ...
    CancelPredicate isCanceled)
{
    ...
    try
    {
        // 第一阶段收集几何点对，同时建立求解结果与原始文件的稳定对应关系
        ...
        for (int i = 0; i < total; ++i)
        {
            ...
            if (!img.empty())
            {
                // 一组内参只对应一个像素坐标尺度，尺寸不同的图片不能混入同次求解
        ...
        double rms = 0.0;
        // 第二阶段按模型选择 OpenCV 求解器，并用选项固定不参与估计的参数
    ...
    QString *error) const
{
    ...
    try
    {
        // 先在内存中完成整个文档，随后通过 QSaveFile 一次性提交到目标路径
        ...
        // FileStorage 先写占位键，释放字符串后再替换为兼容 OpenCV 的矩阵块
        ...
        // 仅替换已定位的占位行，避免手工拼接其余由 OpenCV 生成的 YAML
    ...
    const QString &filePath, QString *error) const
{
    ...
    try
    {
        ...
        // 核心矩阵和分辨率是必需字段，其余元数据缺失时沿用结构体默认值
        CalibrationResult result;
        cv::String cameraModel;
        readIfPresent(storage, "camera_model", cameraModel);
        result.options.cameraModel = cameraModel == "fisheye" ? CameraModel::Fisheye : CameraModel::Pinhole;
